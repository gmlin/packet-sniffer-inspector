...
#def collect(time):
	...
	if os.name == 'nt': # if the machine is a windows machine
		sniffer = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_IP) # create a socket that accepts all TCP datagrams
	...
	file = open("dump_file.txt", 'w+') # create a new file to dump the bytes into
...
def parse(packet):
	return "" # return the parsed information
